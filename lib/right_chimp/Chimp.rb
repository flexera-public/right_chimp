...
$stdout.sync = true
    ...
    def run
      ...
      if Chimp.failure
        ...
        puts "##################################################"
        puts "[#{Chimp.get_job_uuid}] API CALL FAILED FOR:"
        puts "[#{Chimp.get_job_uuid}] chimp #{@cli_args} "
        puts "[#{Chimp.get_job_uuid}] Run manually!"
        puts "##################################################"
        exit 1
      end
      #
    ...
    def get_array_info
      ...
      if all_instances.nil?
        puts "[#{Chimp.get_job_uuid}] No results from API query"
      else
	...
	puts "[#{Chimp.get_job_uuid}] Found #{@servers.count} servers for that array query"
      ...
      if not (@servers.empty? )
        if (@script != nil)
          ...
          if @script =~ /\A#{URI::regexp}\z/
            ...
            puts "[#{Chimp.get_job_uuid}] Making API 1.5 call : client.resource(#{s.params['right_script']['href'].scan(/=(.*)/).last.last})"
            begin
              the_name = Connection.client.resource(s.params['right_script']['href'].scan(/=(.*)/).last.last).name
            rescue
              puts "[#{Chimp.get_job_uuid}] Failed to make 1.5 call for rightscript href"
            end
    ...
    def verify_tagged_instances(servers,tags)
      ...
      if matching_servers.size != servers.size
        if @ignore_errors
          puts "[#{Chimp.get_job_uuid}] #{servers.size - matching_servers.size} instances didnt match tag selection."
          puts "[#{Chimp.get_job_uuid}] #{tags.join(" ")}"
          Chimp.set_failure(true)
          puts "[#{Chimp.get_job_uuid}] Set failure to true because of disccrepancy"
    ...
    def get_hrefs_for_arrays(names)
      ...
      if names.size > 0
        names.each do |array_name|
          ...
          puts "[#{Chimp.get_job_uuid}] Making API 1.5 call: client.server_arrays.index(:filter => [#{array_name}])"
          ...
          if result.size != 0
            ...
            if @ignore_errors
              puts "[#{Chimp.get_job_uuid}] Could not find array \"#{array_name}\""
            else
              puts "[#{Chimp.get_job_uuid}] Could not find array \"#{array_name}\""
            end
          end
        ...
        if ( arrays_hrefs.empty? )
          puts "[#{Chimp.get_job_uuid}] Did not find any arrays that matched!" unless names.size == 1
        end
    ...
    def detect_server_template(servers)
      ...
      puts "[#{Chimp.get_job_uuid}] Looking for server template"
      st = []
      if servers[0].nil?
        return (st)
      end
      ...
      #
      puts "[#{Chimp.get_job_uuid}] Found server templates"
    ...
    def detect_right_script(st, script)
      puts  "[#{Chimp.get_job_uuid}] Looking for rightscript"
      executable = nil
      ...
      if @script == "" and @script != nil
        ...
        @op_scripts.each  do |rb|
          ...
          if script_name.downcase.include?(script.downcase)
            ...
            puts "[#{Chimp.get_job_uuid}] Found rightscript"
        ...
        if @script_to_run == nil
          ...
          if @script_to_run.nil?
            if @interactive
              ...
              puts "["+self.job_uuid+"] Sorry, didnt find the script: ( "+script+" )!"
              return nil
            end
          else
    ...
    def search_for_script_in_sts(script, st)
      ...
      st.each do |s|
        puts "[#{Chimp.get_job_uuid}] Making API 1.5 call: client.resource(#{s[1]['href']})"
        begin
          temp=Connection.client.resource(s[1]['href'])
        rescue
          puts "[#{Chimp.get_job_uuid}] Failed to load href for ST"
        end
        temp.runnable_bindings.index.each do |x|
          # Look for first match
          if x.raw['right_script']['name'].downcase.include?(script.downcase)
            puts "[#{Chimp.get_job_uuid}] Found requested righscript: #{script}"
            # Provide the name + href
            s = Executable.new
    ...
    def extract_operational_scripts(st)
      ...
      st.each do |s|
        ...
        puts "[#{Chimp.get_job_uuid}] Making API 1.5 call: client.resource (ST)"
        begin
          temp=Connection.client.resource(s[1]['href'])
          puts "[#{Chimp.get_job_uuid}] API 1.5 call client.resource (ST) complete"
          temp.runnable_bindings.index.each do |x|
          ...
          puts "[#{Chimp.get_job_uuid}] API 1.5 call client.resource (ST) failed"
    ...
    def generate_jobs(queue_servers, queue_template, queue_executable)
      counter = 0
      tasks = []
      puts "[#{Chimp.get_job_uuid}] Loading queue..."
      #
      # Configure group
      #
      ...
      #
      puts("[#{Chimp.get_job_uuid}] Processing server selection")
      ...
      queue_servers.each do |server|
        ...
        puts "[#{Chimp.get_job_uuid}] Making API 1.5 call: client.resource (SERVER)"
        begin
          s.object = Connection.client.resource(server['href'])
          puts "[#{Chimp.get_job_uuid}] Making API 1.5 call: client.resource (SERVER) COMPLETE"
        rescue
          puts "[#{Chimp.get_job_uuid}] Failed to load server href via API1.5"
        end
    ...
    def get_results(group_name)
      queue = ChimpQueue.instance
      puts("getting results for group #{group_name}")
    ...
    def process
      ...
      puts "[#{Chimp.get_job_uuid}] Processing task"
      puts "["+self.job_uuid+"] Chimp.failure is set to 1: #{Chimp.failure.to_s}"

      puts "[#{Chimp.get_job_uuid}] Trying to get array_info" unless Chimp.failure
      get_array_info unless Chimp.failure

      puts "["+self.job_uuid+"] Chimp.failure is set to 2: #{Chimp.failure.to_s}"

      puts "[#{Chimp.get_job_uuid}] Trying to get server_info" unless Chimp.failure
      get_server_info unless Chimp.failure
      puts "["+self.job_uuid+"] Chimp.failure is set to 3: #{Chimp.failure.to_s}"
      puts "[#{Chimp.get_job_uuid}] Trying to get template_info" unless Chimp.failure
      get_template_info unless Chimp.failure
      puts "["+self.job_uuid+"] Chimp.failure is set to 4: #{Chimp.failure.to_s}"
      puts "[#{Chimp.get_job_uuid}] Trying to get executable_info" unless Chimp.failure
      get_executable_info unless Chimp.failure

      if Chimp.failure

        puts "["+self.job_uuid+"] Chimp.failure is set to 5: #{Chimp.failure.to_s}"
        puts "##################################################"
        puts "["+self.job_uuid+"] API CALL FAILED FOR:"
        puts "["+self.job_uuid+"] chimp #{@cli_args} "
        puts "["+self.job_uuid+"] Run manually!"
        puts "##################################################"
        return []
      else
